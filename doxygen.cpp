	...
	// Here comes the bitch.
		...
		if (!arglist.contains('(')) {
			if (DoxySettings.shortVarDoc) {
				...
				if (DoxySettings.verbosePrinting)
					docToWrite += indent + DoxySettings.DoxyComment.doxNewLine + "var " + overview.prettyName(name) + indent + DoxySettings.DoxyComment.doxEnding;
				else
...
bool Doxygen::addFileComment(const DoxygenSettingsStruct& DoxySettings, Core::IEditor* editor)
{
	...
	text += DoxySettings.DoxyComment.doxBrief;
	text += DoxySettings.DoxyComment.doxEmptyLine;
	text += DoxySettings.fileComment;
	text += DoxySettings.DoxyComment.doxNewLine;
#if defined(Q_OS_WIN)
	text += "author "+ QString::fromUtf8(getenv("USERNAME"));
#elif defined(Q_OS_LINUX) || defined(Q_OS_MAC)
	text += "author "+ QString::fromUtf8(getenv(getenv("USER"));
#endif
	text += "\n"+DoxySettings.DoxyComment.doxNewLine;
