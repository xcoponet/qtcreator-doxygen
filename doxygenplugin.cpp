...
static const char CMD_ID_COMMENTFILE[] = "Doxygen.CommentFile";
...
bool DoxygenPlugin::initialize(const QStringList& arguments, QString* errorString)
{
	...
	//add comment for the current file
	m_doxygenDocumentAddFileCommentAction = new QAction(tr("Add header comment for this file"), this);
	command = am->registerAction(m_doxygenDocumentAddFileCommentAction, CMD_ID_COMMENTFILE, globalcontext);
	command->setAttribute(Core::Command::CA_UpdateText);
	command->setDefaultKeySequence(QKeySequence(tr("Ctrl+Shift+F2")));
	connect(m_doxygenDocumentAddFileCommentAction, SIGNAL(triggered(bool)), this, SLOT(addCurrentFileComment()));
	doxygenMenu->addAction(command);
...
*/
	...
	connect(this, SIGNAL(doxyCurrentAddFileComment(DoxygenSettingsStruct, Core::IEditor*)),
		dox, SLOT(addFileComment(DoxygenSettingsStruct, Core::IEditor*)));
...
{
	Core::IEditor* editor = Core::EditorManager::instance()->currentEditor();
	emit doxyDocumentCurrentProject(settings(),editor);
}

void DoxygenPlugin::addCurrentFileComment()
{
	Core::IEditor* editor = Core::EditorManager::instance()->currentEditor();
	emit doxyCurrentAddFileComment(settings(), editor);
}
